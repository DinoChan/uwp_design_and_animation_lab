...
namespace DesignAndAnimationLab.Demos.Gooey
{
    ...
    public sealed partial class TextMorph : Page
    {
        ...
        private readonly string[] _texts = { "Why", "is", "this", "so", "satisfying", "to", "watch?" };
        ...
        {
            _blurEffect = new GaussianBlurEffect { BlurAmount = 0f };
        ...
        private void OnDraw(ICanvasAnimatedControl sender, CanvasAnimatedDrawEventArgs args)
        {
            ...
            using (var drawingSession = source.CreateDrawingSession())
            {
                foreach (var item in _morphItems)
                {
                    ...
                    drawingSession.DrawText(item.Text, _centerPoint,
                        new CanvasSolidColorBrush(sender, Colors.Black) { Opacity = Convert.ToSingle(progress) },
                        _textFormat);
                }
            }
